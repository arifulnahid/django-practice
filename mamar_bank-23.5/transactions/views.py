...
def mail_sender(subject, to, context):
    template = 'transactions/mail.html'
    convert_to_html_content = render_to_string(
        template_name=template, context=context)
    plain_message = strip_tags(convert_to_html_content)

    send = send_mail(subject=subject, message=plain_message,
                     from_email='771a0f001@smtp-brevo.com', recipient_list=['mdnahid6530@gmail.com'], html_message=convert_to_html_content)
    if send:
...
class SendMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        self.model.objects.create(
            account=receiver, amount=amount, transaction_type=RECEIVE_MONEY, balance_after_transaction=receiver.balance)
        mail_sender('Send Money', 'ainahidfsti@gmail.com', {'amount': amount})

        messages.success(
            self.request,
            f'Successfully send {"{:,.2f}".format(
                float(amount))}$ from your account'
        )
